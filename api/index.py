"""Reversion Lab — Vercel-ready FastAPI backend"""
...
def twelve_data_daily(symbol: str, outputsize: int = 500) -> pd.DataFrame:
    if not TWELVE_DATA_KEY:
        raise HTTPException(500, detail="TWELVE_DATA_KEY not set")
    r = requests.get("https://api.twelvedata.com/time_series", timeout=30, params={
        "symbol": symbol, "interval": "1day", "outputsize": outputsize, "order": "ASC", "apikey": TWELVE_DATA_KEY})
    r.raise_for_status()
    raw = r.json()
    vals = raw.get("values", [])
    if not vals:
...
def polygon_daily(symbol: str, start: str, end: str) -> pd.DataFrame:
    if not POLYGON_KEY:
        raise HTTPException(500, detail="POLYGON_KEY not set")
    r = requests.get(
...
def backtest(signals: pd.DataFrame, symbol: str, cfg: ScanRequest):
    ...
    peak = equity; max_dd = 0.0; i = 0
    while i < len(signals) - 1:
        row = signals.iloc[i]
        if int(row.get("signal", 0)) != 1:
            curve.append({"date": str(row["date"].date()), "equity": equity}); i += 1; continue
        entry_idx = i + 1
        if entry_idx >= len(signals): break
        entry_row = signals.iloc[entry_idx]
        entry = float(entry_row["close"]); stop = float(entry_row["low"])
        ...
        trades.append({
            "symbol": symbol, "signal_date": str(row["date"].date()),
            "entry_date": str(entry_row["date"].date()), "exit_date": str(exit_row["date"].date()),
...
def portfolio_analytics(df: pd.DataFrame) -> dict:
    if df.empty: return {}
    wins = df[df["net_return"] > 0]; losses = df[df["net_return"] <= 0]
    gw = wins["net_return"].sum(); gl = losses["net_return"].abs().sum()
...
def persist_symbol(symbol, verified, signals, trades_df):
    conn = get_conn()
    def d(v): return str(v.date()) if hasattr(v,"date") else str(v)
    conn.executemany("INSERT OR REPLACE INTO verification_audit VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
    ...
    conn.executemany("INSERT OR REPLACE INTO signals VALUES (?,?,?,?,?,?,?)",
        [(symbol, d(r["date"]),
          ...
          int(r["verified"]), int(r["signal"]), float(r["close"])) for _, r in signals.tail(500).iterrows()])
    if not trades_df.empty:
        conn.executemany("""INSERT INTO trades
               ...
               VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)""",
            [(r["symbol"],r["signal_date"],r["entry_date"],r["exit_date"],r["entry_price"],r["exit_price"],
              r.get("shares"),r.get("dollar_risk"),r["gross_return"],r["net_return"],r.get("pnl_dollars"),
              int(r["bars_held"]),int(r["verified"]),r["ibs"],r.get("range_factor"),r["equity_after"])
...
def submit_alpaca_order(symbol, qty, signal_date):
    ...
    r = requests.post(f"{ALPACA_BASE_URL}/v2/orders", json=payload, headers=headers, timeout=15)
    resp = r.json()
    conn = get_conn()
    conn.execute("INSERT OR REPLACE INTO broker_orders VALUES (?,?,?,?,?,?,?,?,?)",
...
def run_scan(cfg: ScanRequest):
    ...
    for symbol in symbols:
        try:
            ...
            start = str(primary["date"].min().date()); end = str(primary["date"].max().date())
            ...
            latest = signals.iloc[-1]; latest_v = verified.iloc[-1]
            sig_val = int(latest["signal"]); ver_val = int(latest["verified"])
            broker_resp = None
            if sig_val == 1 and ver_val == 1 and cfg.auto_execute:
                entry = float(primary["close"].iloc[-1]); stop = float(primary["low"].iloc[-1])
                sz = calc_position_size(entry, stop, cfg.account_equity, cfg.risk_frac)
                if sz["shares"] > 0:
                    broker_resp = submit_alpaca_order(symbol, sz["shares"],
...
@app.get("/api/health")
def health(): return {"ok": True, "time": datetime.utcnow().isoformat(), "version": "1.0.0"}

@app.get("/api/config", dependencies=[Depends(require_auth)])
def get_config():
    return {"symbols": DEFAULT_SYMBOLS, "primary_source": "TWELVE_DATA",
...
def cron_eod_scan(authorization: Optional[str] = None):
...
def dashboard_summary():
    ...
    return {"signals_count": sum(1 for s in latest_signals if s.get("signal")==1),
...
def verification(symbol: str, limit: int = Query(50,ge=1,le=500)):
    conn = get_conn()
    rows = conn.execute("SELECT * FROM verification_audit WHERE symbol=? ORDER BY date DESC LIMIT ?", (symbol.upper(), limit)).fetchall()
    conn.close()
...
def add_journal(trade: JournalTrade):
    ...
    conn.execute("""INSERT INTO trade_journal
        (symbol,trade_date,side,entry_price,exit_price,shares,r_multiple,source_1_close,source_2_close,verified,notes)
        VALUES (?,?,?,?,?,?,?,?,?,?,?)""",
        (trade.symbol.upper(),trade.trade_date,trade.side,trade.entry_price,trade.exit_price,
         trade.shares,trade.r_multiple,trade.source_1_close,trade.source_2_close,trade.verified,trade.notes))
    conn.commit(); conn.close()
    return {"ok": True}
...
def export_trades():
    conn = get_conn(); df = pd.read_sql("SELECT * FROM trades ORDER BY trade_id ASC", conn); conn.close(); return _csv(df, "trades.csv")

@app.get("/api/export/signals.csv", dependencies=[Depends(require_auth)])
def export_signals():
    conn = get_conn(); df = pd.read_sql("SELECT * FROM signals ORDER BY date ASC", conn); conn.close(); return _csv(df, "signals.csv")

@app.get("/api/export/verification.csv", dependencies=[Depends(require_auth)])
def export_verification():
    conn = get_conn(); df = pd.read_sql("SELECT * FROM verification_audit ORDER BY date ASC", conn); conn.close(); return _csv(df, "verification_audit.csv")

@app.get("/api/export/journal.csv", dependencies=[Depends(require_auth)])
def export_journal():
    conn = get_conn(); df = pd.read_sql("SELECT * FROM trade_journal ORDER BY journal_id ASC", conn); conn.close(); return _csv(df, "journal.csv")

@app.get("/api/export/portfolio-analytics.csv", dependencies=[Depends(require_auth)])
def export_portfolio():
    conn = get_conn(); df = pd.read_sql("SELECT * FROM trades", conn); conn.close()
    a = portfolio_analytics(df)
    return _csv(pd.DataFrame(a.get("by_symbol", [])), "portfolio_analytics.csv")
